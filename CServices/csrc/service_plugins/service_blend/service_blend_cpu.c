...
run_service_blend_cpu(PixelPacket *pixpack_target, PixelPacket *pixpack_source1, PixelPacket *pixpack_source2, int32_t rows1, int32_t columns1, int32_t rows2, int32_t columns2, float dirX, float dirY, float x, float y, int32_t mode) {
	...
	if (mode < 10) {
		if (mode == 0) // simple blend - linear
			#pragma omp parallel
		{
			#pragma omp for private(j, pos_ij, pos_t_ij)
			for (i = 0; i < rows1; ++i) {
