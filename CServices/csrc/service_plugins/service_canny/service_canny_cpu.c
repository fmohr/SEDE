...
int32_t run_service_canny_cpu(PixelPacket *pixpack_target, PixelPacket *pixpack_source, int32_t rows, int32_t columns, float Tlow, float Thigh) {
	int32_t i, j, pos;
	float pixangle;
	...
	#pragma omp parallel
	{
		#pragma omp for private(j, pos, temp_red, temp_green, temp_blue)
		for (i = 2; i < rows - 2; ++i) {
		...
		#pragma omp for private (sobelX, sobelY, pixangle)
