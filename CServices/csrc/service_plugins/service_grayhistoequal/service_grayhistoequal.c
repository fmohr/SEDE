/* service_grayhistoequal.c */

#include "service_grayhistoequal.h"

typedef int32_t (*run_service_grayhistoequal_gpu_t)(PixelPacket *, PixelPacket *, int32_t, int32_t);
...
void run_service(char resource, void *handler_gpu, Image **target_images, int32_t *target_images_size, Image *source_images, int32_t source_images_size, double *params, int32_t *status_code) {
	...
	*target_images_size			= 1;
	...
	run_service_grayhistoequal_gpu_t run_service_grayhistoequal_gpu;
	char *dlsym_error;
	
	switch (resource) {
		case 'c':
			*status_code = run_service_grayhistoequal_cpu((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns);
			break;
		case 'g':
			#ifdef PGCC
			handler_gpu = dlopen("../bin/plugins/libservice_grayhistoequal_gpu.so", RTLD_LAZY | RTLD_GLOBAL);
			if (NULL == handler_gpu) {
				fprintf(stderr, "Cannot open library libservice_grayhistoequal_gpu.so: %s (file: %s, line: %i)\n", dlerror(), __FILE__, __LINE__);
				exit(EXIT_FAILURE);
			}
			#endif
			run_service_grayhistoequal_gpu = (run_service_grayhistoequal_gpu_t) dlsym(handler_gpu, "run_service_grayhistoequal_gpu");
			dlsym_error = dlerror();
			if (dlsym_error) {
				fprintf(stderr, "Cannot load symbol \"run_service_grayhistoequal_gpu\": %s (file: %s, line: %i)\n", dlsym_error, __FILE__, __LINE__);
				break;
			}
			*status_code = run_service_grayhistoequal_gpu((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns);
			#ifdef PGCC
			dlclose(handler_gpu);
			#endif
			break;
		case 'f':
			*status_code = run_service_grayhistoequal_fpga((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns);
			break;
		case 's':
		default:
			*status_code = run_service_grayhistoequal_scpu((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns);
			break;
	}
}
...
void get_service_info(char ***info_strings, int32_t *info_strings_size) {
	char *strings[] = {
		"Histogram equalization for gray scale values."
	};
	*info_strings_size = 1;
