...
int32_t run_service_radialblur_cpu(PixelPacket *pixpack_target, PixelPacket *pixpack_source, int32_t rows, int32_t columns) {
	...
	int32_t la_pos, a_pos, ra_pos, l_pos, r_pos, lu_pos, u_pos, ru_pos;
	double theta;
	#pragma omp parallel
	{
		#pragma omp for private(j, pos, cartx, carty, rad, src_x, src_y, newpos, rasterx, rastery, theta)
		for (i = 0; i < rows; i++) {
			for (j = 0; j < columns; j++) {
				pos = i * columns + j;
				cartx = j - centerx;
	...
	int32_t n;
	for (n = 0; n < 4; n++) {
		#pragma omp for private(j, la_pos, a_pos, ra_pos, l_pos, r_pos, lu_pos, u_pos, ru_pos, temp_red, temp_green, temp_blue)
		for (i = 1; i < rows - 1; i++) {
