...
#pragma acc loop independent
		for (y = 0; y < rows; ++y) {
#pragma acc loop independent private(newX, newY, oldPosition, newPosition, finalY, finalX)
			for (x = 0; x < columns; ++x) {
				...
				//rotation + shift
				int32_t finalY = (int32_t) ((newY * cos_angle - newX * sin_angle) + yOffset + 0.5);
				int32_t finalX = (int32_t) ((newY * sin_angle + newX * cos_angle) + xOffset + 0.5);
