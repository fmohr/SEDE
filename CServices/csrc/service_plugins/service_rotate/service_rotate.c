...
typedef int32_t (*run_service_rotate_gpu_t)(PixelPacket *, PixelPacket *, int32_t, int32_t, float);
...
void run_service(char resource, void *handler_gpu, Image **target_images, int32_t *target_images_size, Image *source_images, int32_t source_images_size, double *params, int32_t *status_code) {
	...
	*target_images_size			= 1;
	...
	run_service_rotate_gpu_t run_service_rotate_gpu;
	char *dlsym_error;
	
	switch (resource) {
		case 'c':
			*status_code = run_service_rotate_cpu((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns, (float)params[0]);
			break;
		case 'g':
			#ifdef PGCC
			handler_gpu = dlopen("../bin/plugins/libservice_rotate_gpu.so", RTLD_LAZY | RTLD_GLOBAL);
			if (NULL == handler_gpu) {
				fprintf(stderr, "Cannot open library libservice_rotate_gpu.so: %s (file: %s, line: %i)\n", dlerror(), __FILE__, __LINE__);
				exit(EXIT_FAILURE);
			}
			#endif
			run_service_rotate_gpu = (run_service_rotate_gpu_t) dlsym(handler_gpu, "run_service_rotate_gpu");
			dlsym_error = dlerror();
			if (dlsym_error) {
				fprintf(stderr, "Cannot load symbol \"run_service_rotate_gpu\": %s (file: %s, line: %i)\n", dlsym_error, __FILE__, __LINE__);
				break;
			}
			*status_code = run_service_rotate_gpu((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns, (float)params[0]);
			#ifdef PGCC
			dlclose(handler_gpu);
			#endif
			break;
		case 'f':
			*status_code = run_service_rotate_fpga((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns, (float)params[0]);
			break;
		case 's':
		default:
			*status_code = run_service_rotate_scpu((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns, (float)params[0]);
			break;
	}
}
