...
public class GradleProjectBuild extends AbstractProcessBuild{
    ...
    public GradleProjectBuild(String displayName, DynType buildDef) {
        super(displayName, buildDef);
        gradleProcess = getBuildDef().cast(GradleProcess.class);
    }

    @Override
    protected void configureProcess(DefaultProcessHandleBuilder builder, ServiceDeployment current) {
        DeploymentMethodStepWorkdir workDirDef = getBuildDef().cast(DeploymentMethodStepWorkdir.class);
