...
If `merge` is not enabled, each input file is considered individually. When the scripts build upon, enable `merge` to merge the input files into a single output.
...
java -jar sdlc.jar \
    -m <1>
    -i MathLib-1.servicedesc.groovy MathLib-2.servicedesc.groovy <2>
    -o MathLib.servicedesc.json <3>
----
<1> Merge one or more inputs
<2> Multiple input files
<3> Single output, which contains the merged result

NOTE: The sdl file of the previous example is reused. This way the second service description merely updates the `MathLib` collection and retains the info block. Note that the order of the input files matters.
The resulting json file now becomes:

[source, json]
.MathLib.servicedesc.json
----
include::MathLib-2.servicedesc.json[]
----
