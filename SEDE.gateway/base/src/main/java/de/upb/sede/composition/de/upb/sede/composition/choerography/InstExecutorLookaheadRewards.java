...
public class InstExecutorLookaheadRewards extends
    ...
    private void selectInstHost() {
        ...
        if(getCurrentInstruction().getInstruction().getContextIsFieldFlag()) {
            ...
            Optional<Long> prevAccess = getInput().getFieldAccessExplorer().getPrevAccess(getCurrentInstruction().getIndex(), access -> access.getField().equals(contextFieldName) && access.getAccessType().isAssignOrWrite());
            if(prevAccess.isEmpty()) {
    ...
    private void rewardExecutors() {
        ...
        Iterator<IFieldAccess> readAccesses = getInput()
            .getFieldAccessExplorer().accessesAtInst(
            instIndex,
