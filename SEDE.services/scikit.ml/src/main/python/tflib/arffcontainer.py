    ...
    try:
        try:
            if dense_mode:
                ...
                mode = larff.LOD
            arff_parsed = larff.load(arff_data, return_type=mode, encode_nominal=True)
        except (KeyboardInterrupt, SystemExit):
            raise
...
class ArffStructure:
    ...
    `Args`: 
        ...
        arff_data: arff_data which is a dictionary returned by the larff.py module
    ...
    def from_arff(cls, arff_data):
        """ Reads and encapsulates arff_data which is a dictionary returned by the larff.py module.
        """
