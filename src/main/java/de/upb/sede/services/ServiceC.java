...
public class ServiceC extends Service {
	...
	private volatile Map<String, ServiceInstanceC> instances = new HashMap<>();
	private long mHandler;
	private long mHandlerGPU;

	/**
	 * In the process of constructing this java representation of a plug-in the JNI
	 * calls are used to receive all kind of information about the plug-in which is
	 ...
	 * 
	 * @param linkedFiles
	 *            The various implementations for different resources.
	 * 
	 */
	public ServiceC(File serviceFile, List<File> linkedFiles) {
	 ...
	 * 
	 */
	public ServiceC(File serviceFile) {
		super(serviceFile);
		init();
	}
	...
	/* METHODS */
	public Object process(char resource, List<Double> params, List<Image> images) {
	...
	public ServiceInstanceC getInstance(String key) {
	 ...
	 */
	protected native List<Image> c_process(long handler, long handlerGPU, char resource, List<Double> params,
			List<Image> images);
